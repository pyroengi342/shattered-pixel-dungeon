...
import type { ToolDescriptor, ToolDescriptorFromSchema, ToolListItem, ToolRawResult } from './tool.js';
/**
...
export interface ModelContextTesting {
    ...
    executeTool(toolName: string, inputArgsJson: string, options?: ModelContextTestingExecuteToolOptions): Promise<string | null>;
 ...
 */
export type InferToolArgs<TTool> = TTool extends ToolDescriptor<infer TArgs, infer _TResult, infer _TName> ? TArgs : never;
/**
 * Infers result shape from a tool descriptor.
 */
export type InferToolResult<TTool> = TTool extends ToolDescriptor<infer _TArgs, infer TResult, infer _TName> ? TResult : never;
/**
 * Union of tool names from a tuple/array of descriptors.
 */
export type ToolName<TTools extends readonly {
    name: string;
}[]> = TTools[number]['name'];
/**
 * Extracts a tool descriptor by name from a tuple/array.
 */
export type ToolByName<TTools extends readonly {
    name: string;
}[], TName extends ToolName<TTools>> = Extract<TTools[number], {
    name: TName;
}>;
/**
 * Tool argument type by tool name from a tuple/array.
 */
export type ToolArgsByName<TTools extends readonly {
    name: string;
}[], TName extends ToolName<TTools>> = InferToolArgs<ToolByName<TTools, TName>>;
/**
 * Tool result type by tool name from a tuple/array.
 */
export type ToolResultByName<TTools extends readonly {
    name: string;
}[], TName extends ToolName<TTools>> = InferToolResult<ToolByName<TTools, TName>>;
/**
 * Typed parameters for `callTool`.
 *
 * When a tool has no args (`Record<string, never>`), `arguments` becomes optional.
 */
export type ToolCallParams<TName extends string = string, TArgs extends Record<string, unknown> = Record<string, never>> = {
    name: TName;
...
export interface ModelContextCore {
     ...
     */
    registerTool<TInputSchema extends JsonSchemaForInference, TOutputSchema extends JsonSchemaForInference | undefined = undefined, TName extends string = string>(tool: ToolDescriptorFromSchema<TInputSchema, TOutputSchema, TName>): void;
    /**
     * Registers a dynamic tool with explicitly typed args/result.
     */
    registerTool<TInputSchema extends InputSchema, TArgs extends Record<string, unknown> = Record<string, unknown>, TName extends string = string>(tool: ToolDescriptor<TArgs, ToolRawResult, TName> & {
        inputSchema: TInputSchema;
    } & (TInputSchema extends InputSchema ? string extends TInputSchema['type'] ? unknown : never : unknown)): void;
    /**
     * Registers a dynamic tool without an explicit inputSchema.
     * Runtime defaults this to an empty object schema.
     */
    registerTool<TArgs extends Record<string, unknown> = Record<string, unknown>, TName extends string = string>(tool: Omit<ToolDescriptor<TArgs, ToolRawResult, TName>, 'inputSchema'> & {
...
export interface ModelContextExtensions {
     ...
     */
    callTool<TName extends string = string, TArgs extends Record<string, unknown> = Record<string, unknown>>(params: {
        name: TName;
        arguments?: TArgs;
     ...
     */
    addEventListener(type: 'toolschanged', listener: () => void, options?: boolean | AddEventListenerOptions): void;
    /**
     ...
     */
    removeEventListener(type: 'toolschanged', listener: () => void, options?: boolean | EventListenerOptions): void;
    /**
     * Dispatches an event.
     */
 ...
 */
export type TypedModelContext<TTools extends readonly {
    name: string;
}[] = readonly ToolDescriptor[]> = ModelContextWithExtensions & {
    /**
     * Executes a known tool with name-aware argument and response inference.
     */
    callTool<TName extends ToolName<TTools>>(params: ToolCallParams<TName, ToolArgsByName<TTools, TName>>): Promise<ToolResultByName<TTools, TName>>;
    /**
