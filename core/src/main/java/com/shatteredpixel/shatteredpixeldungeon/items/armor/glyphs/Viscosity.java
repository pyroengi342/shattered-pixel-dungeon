...
public class Viscosity extends Glyph {
	...
	public int proc(Armor armor, Char attacker, Char defender, int damage) {
		Buff.affect(defender, ViscosityTracker.class, this).level = armor.buffedLvl();
		return damage;
	}
	...
	public static class ViscosityTracker extends Buff {
		...
		public int deferDamage(int dmg) {
			...
			percent *= genericProcChanceMultiplier(target, (target instanceof Hero) ? (Hero) target : null);
			...
			if (amount > 0) {
				DeferedDamage deferred = Buff.affect(target, DeferedDamage.class, this);
	...
	public static class DeferedDamage extends Buff {
		...
		public void extend(float damage) {
			if (this.damage == 0) postpone(TICK);
			this.damage += damage;
		}
