...
public class WandOfTransfusion extends DamageWand {
	...
	public void onZap(Ballistica beam) {
		...
		if (ch instanceof Mob){
			...
			if (ch.alignment == Char.Alignment.ALLY || ch.buff(Charm.class) != null){
				...
				if (shielding > 0){
					healing -= shielding;
					Buff.affect(ch, Barrier.class, this).setShield(shielding);
				} else {
					shielding = 0;
				}
			...
			//(or for mimics which are hiding, special case)
				...
				Buff.affect(curUser, Barrier.class, this).setShield((5 + buffedLvl()));
	...
	public void onHit(MagesStaff staff, Char attacker, Char defender, int damage) {
		if (defender.buff(Charm.class) != null && defender.buff(Charm.class).object == attacker.id()){
			...
			Buff.affect(attacker, Barrier.class, this).setShield(shieldToGive);
	...
	public String statsDesc() {
		int selfDMG = curUser != null ? Math.round(curUser.HT*0.05f): 1;
		if (levelKnown)
