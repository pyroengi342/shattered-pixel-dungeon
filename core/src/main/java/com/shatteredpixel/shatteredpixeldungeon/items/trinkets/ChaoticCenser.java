...
//        int level = isIdentified() ? buffedLvl() : 0;
    ...
    public int averageTurnsUntilGas() {
        int lvl = trinketLevel(ChaoticCenser.class, curUser);
        if (lvl == -1) return -1;
        return 300 / (lvl + 1);
    }
    ...
    public static class CenserGasTracker extends Buff {
        ...
        public boolean act() {
            ...
            ChaoticCenser censer = null;
            Object src = getSource();
            if (src instanceof ChaoticCenser) {
                censer = (ChaoticCenser) src;
            } else {
                spend(TICK);
                return true;
            }

            int avgTurns = censer.averageTurnsUntilGas();
            if (avgTurns == -1) {
            ...
            if (left <= 0) {
                if (owner.visibleEnemies() > 0) {
                    Char target = owner.visibleEnemy(0);
                    if (target != null && target.isActive() && target.alignment == Char.Alignment.ENEMY
                            && (!(target instanceof Mob) || ((Mob) target).state != ((Mob) target).PASSIVE)) {
                        if (censer.produceGas(target)) {
    ...
    private boolean produceGas(Char target) {
        int level = trinketLevel(ChaoticCenser.class, curUser);
        ...
        if (Dungeon.level.trueDistance(target.pos, curUser.pos) >= 4) {
            for (int i : PathFinder.NEIGHBOURS8) {
                int next = targetPos + i;
                while (!Dungeon.level.solid[next]
                        && Dungeon.level.trueDistance(next, curUser.pos) < Dungeon.level.trueDistance(targetPos, curUser.pos)) {
                    targetPos = next;
                }
            }
        }
        ...
        if (!candidateCells.isEmpty()) {
            ...
            if (targetCell != null) {
                Buff.affect(curUser, GasSpewer.class, this).set(targetCell, gasToSpawn, (int) gasQuantity);
    ...
    public static class GasSpewer extends FlavourBuff {
        ...
        public boolean act() {
            ...
            if (depth == Dungeon.depth && branch == Dungeon.branch) {
                ...
                if (censer.curUser != null) {
                    MagicMissile.boltFromChar(censer.curUser.sprite.parent, MISSILE_VFX.get(gasType), censer.curUser.sprite, targetCell, null);
                }
