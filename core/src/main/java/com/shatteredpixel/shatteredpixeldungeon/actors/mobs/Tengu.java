...
public class Tengu extends Mob {
	...
	public void damage(int dmg, Object src) {
		...
		// Определяем героя, нанёсшего урон (если это герой)
		Hero attackerHero = null;
		if (src instanceof Hero) {
			attackerHero = (Hero) src;
		}else if (src instanceof Char) {
			Char attackerChar = (Char) src;
			if (attackerChar.alignment == Alignment.ALLY && attackerChar instanceof DirectableAlly) {
				attackerHero = ((DirectableAlly) attackerChar).getOwner();
			}
//			else if (src instanceof Item) {
//				attackerHero = ((Item) src).curUser;
//			}
		}

		if (attackerHero != null) {
			LockedFloor lock = attackerHero.buff(LockedFloor.class);
...
//			killer = ((Item) cause).curUser;
//		}

		if (killer != null && killer.subClass == HeroSubClass.NONE) {
	...
	private void jump() {
		...
		if (enemy == null) return; // нет цели – прыгать некуда
	...
	public void notice() {
		...
		if (!BossHealthBar.isAssigned()) {
			...
			// приветствие адресовано локальному игроку (визуальное сообщение)
			if (HP == HT) {
	...
	public static boolean throwBomb(final Char thrower, final Char target){
		...
		final BombAbility.BombItem item = new BombAbility.BombItem(thrower);
	...
	public static class BombAbility extends Buff {
		...
		public static class BombItem extends Item {
			...
			protected void onThrow(int cell) {
				super.onThrow(cell);
				Buff.append(thrower, BombAbility.class, null).bombPos = cell;
			}
	...
	public static boolean throwShocker(final Char thrower, final Char target){
		...
		for (int i : PathFinder.NEIGHBOURS8){
			...
			if (Dungeon.level.distance(cell, thrower.pos) >= 2 && !Dungeon.level.solid[cell]){
				...
				if (validTarget && (targetCell == -1 || Dungeon.level.trueDistance(cell, thrower.pos) < Dungeon.level.trueDistance(targetCell, thrower.pos))){
					targetCell = cell;
				}
			}
		}
		...
		final int finalTargetCell = targetCell;
		final ShockerAbility.ShockerItem item = new ShockerAbility.ShockerItem(thrower);
		thrower.sprite.zap(finalTargetCell);
		((MissileSprite) thrower.sprite.parent.recycle(MissileSprite.class)).
				reset(thrower,
						thrower.sprite,
	...
	public static class ShockerAbility extends Buff {
		...
		public static class ShockerItem extends Item {
			...
			public ShockerItem(Char thrower) {
				this.thrower = thrower;
			}
			...
			protected void onThrow(int cell) {
				super.onThrow(cell);
				Buff.append(thrower, ShockerAbility.class, null).shockerPos = cell;
			}
